...
import { FacebookHoverCard } from "@/components/Hero/FacebookHoverCard";
...
function HeroContent() {
  ...
  const [pillHovered, setPillHovered] = useState<"fb" | "gh" | "li" | null>(null);
  ...
  const pillWrapperRefs = { fb: fbWrapperRef, gh: ghWrapperRef, li: liWrapperRef };
  ...
  const hoverPill = (target: "fb" | "gh" | "li", e: React.MouseEvent) => {
    ...
    >
      <motion.div className="flex flex-1 flex-col items-center justify-start px-6 pt-16" style={{ paddingBottom: pb }}>
        ...
        >
          ...
          <LogoBadge id="donoro" label="Donoro" src="/badges/donoro.png" imageSrc="/badges/donoro-preview.png" iframeSrc="https://donoroapp.com/" autoScrollWeb={true} href="https://donoroapp.com/" width={290} active={logoHovered === "donoro"} dimmed={logoHovered !== null && logoHovered !== "donoro"} onHoverChange={setLogoHovered}>
            <span className="whitespace-nowrap">
              ...
              <span className="font-medium text-black dark:text-white">Donoro</span>
            </span>
          </LogoBadge>{" "}
          <span className={`transition-[filter] duration-300 ${logoHovered ? "blur-[8px]" : ""}`}> building{"\n"}emergency blood donation platforms and systems.</span>
        </motion.p>
        ...
        >
          <div>
            <Button
              ...
              onClick={async () => {
                try {
                  await navigator.clipboard.writeText("hello@tamal.dev");
                } catch {}
                setMailCopied(true);
            ...
            >
              ...
              <ActionSwapCascadeText value={mailCopied ? "Copied!" : "hello@tamal.dev"} />
            </Button>
          </div>
          <motion.div
            layout="position"
            transition={{ type: "spring", stiffness: 420, damping: 32, mass: 0.6 } as any}
            ref={fbWrapperRef as any}
            className="relative inline-flex"
            onMouseEnter={(e) => hoverPill("fb", e)}
        ...
        <AnimatePresence>
          {pillHovered && (
            <motion.div
              initial={{ opacity: 0, y: pillCardY + 16, filter: "blur(12px)", x: pillOffset, height: pillHovered === "gh" ? 144 : 175 }}
              animate={{ opacity: 1, y: pillCardY, filter: "blur(0px)", x: pillOffset, height: pillHovered === "gh" ? 144 : 175 }}
            ...
            >
              <AnimatePresence mode="popLayout" initial={false}>
                <motion.div
                  key={pillHovered}
                  initial={pillHovered === "fb" ? { opacity: 0, filter: "blur(6px)" } : false}
                  animate={{ opacity: 1, filter: "blur(0px)" }}
                  exit={pillHovered === "fb" ? { opacity: 0, filter: "blur(6px)" } : { opacity: 0, filter: "blur(0px)" }}
                  transition={pillHovered === "fb" ? { duration: 0.16, ease: "easeOut" } : { duration: 0 }}
                >
                  {pillHovered === "fb" ? <FacebookHoverCard /> : pillHovered === "gh" ? <GitHubHoverCard /> : <LinkedInHoverCard />}
                </motion.div>
              </AnimatePresence>
            </motion.div>
          )}
