    ...
    <section className="w-full" onMouseLeave={() => setHovered(null)}>
      ...
      <div
        className={`mt-[14px] border-t border-dashed transition-colors duration-500 ${EASE} ${
          hovered === content.funProjects[0]?.name ? "border-transparent" : "border-neutral-200 dark:border-neutral-800"
        }`}
      >
        {content.funProjects.map((item, i) => {
          const lineHidden = hovered === item.name || hovered === content.funProjects[i + 1]?.name;
          return (
          <a
            key={item.name}
            href={item.url}
            ...
            className={`group relative grid grid-cols-[48px_1fr] items-center border-b border-dashed py-[7px] pr-[10.5px] transition-colors duration-500 ${EASE} ${
